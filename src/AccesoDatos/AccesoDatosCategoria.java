...
public class AccesoDatosCategoria {
    ...
    public int BuscarIdCategoria(Connection con, String categoria) {
        String sql = "SELECT CAT_ID_CATEGORIA FROM categoria WHERE CAT_NOMBRE_CATEG='" + categoria + "'";
        int idCategoria = 0;                                                        
        try {
            Statement st = con.createStatement();
            ResultSet resultado = st.executeQuery(sql);
            if (resultado.next()) {
                idCategoria = resultado.getInt("CAT_ID_CATEGORIA");
            }
        } catch (Exception e) {
    ...
    public String BuscarCodigoCategoria(Connection con, int categoria) {
        String sql = "SELECT CAT_CODIGO_CATEG FROM categoria WHERE CAT_ID_CATEGORIA=" + categoria + "";
        String CodigoCategoria = "";                                                        
        try {
            Statement st = con.createStatement();
            ResultSet resultado = st.executeQuery(sql);
            if (resultado.next()) {
                CodigoCategoria = resultado.getString("CAT_CODIGO_CATEG");
            }
        } catch (Exception e) {
